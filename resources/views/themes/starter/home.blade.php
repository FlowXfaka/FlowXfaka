                ...
                <div class="catalog-products-stack" data-product-body>
                    @foreach ($categories as $category)
                        <section class="catalog-products-panel{{ $category['is_active'] ? ' is-active' : '' }}" data-category-panel="{{ $category['slug'] }}">
                            @include(\App\Support\StorefrontTheme::view('partials.storefront-products', $storefrontThemeKey), ['products' => $category['products']])
                        </section>
                    @endforeach

                    @if (empty($categories))
                        <section class="catalog-products-panel is-active">
                            @include(\App\Support\StorefrontTheme::view('partials.storefront-products', $storefrontThemeKey), ['products' => $emptyProducts])
                        </section>
                    @endif
                </div>
