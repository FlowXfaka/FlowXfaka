(() => {
    ...
    const animateCatalogPanel = (selectedCategorySlug) => {
        const activePanel = document.querySelector(`[data-category-panel="${selectedCategorySlug}"]`) || document.querySelector('[data-category-panel].is-active');
        if (!activePanel) {
            return;
        }
    ...
    const navigateCatalog = (url) => {
        ...
        setActiveCatalogPanel(requestedCategorySlug);
        animateCatalogPanel(requestedCategorySlug);
    ...
    const applyStorefrontSearch = () => {
        ...
        if (!query) {
            panels.forEach((panel) => {
                Array.from(panel.querySelectorAll('[data-product-card]')).forEach((card) => {
                    card.hidden = false;
                });
            });
            if (emptyState) {
        ...
        const allPanel = document.querySelector('[data-category-panel=""]');
        if (!allPanel) {
            return;
        }
    ...
    document.addEventListener('click', (event) => {
        ...
        if (link.matches(CATEGORY_PILL_SELECTOR) && isCatalogNavigation(nextUrl)) {
            ...
            navigateCatalog(nextUrl.toString());
            return;
        }
